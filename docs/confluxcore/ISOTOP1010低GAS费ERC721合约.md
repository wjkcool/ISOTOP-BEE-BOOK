...
另外写Dapp的前端，使用ISOTOP1010合约时，需要用到` js-conflux-sdk`。
...
2.  通过**工厂合约地址**获得**工厂合约对象**
 ...
 ```javascript
    ...
    const receipt = await  Factory.deployContract("ISOTOP1010").sendTransaction({ from:  account }).executed();
...
## 3.  查询子合约地址并获得`IISOTOP`对象
  ...
  ```js
      ...
      const IISOTOP1010Addr = format.address((IISOTOP1010AddrArray[IISOTOP1010AddrArray.length-1]）,1);  //这里需要将hex40地址格式转换成conflux的cip37地址格式
  ```
