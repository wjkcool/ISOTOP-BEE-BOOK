...
另外写Dapp的前端，使用ISOTOP1011合约时，需要用到` js-conflux-sdk`，`Buffer`和anyweb的`Provider`。
...
```js
    ...
    let  data = Factory.deployContract('ISOTOP1011').data;
...
## 3.  查询子合约地址并获得`IISOTOP`对象
  ...
  ```js
      ...
      const IISOTOP1011Addr = format.address((IISOTOP1011AddrArray[IISOTOP1011AddrArray.length-1]）,1029);  //这里需要将hex40地址格式转换成conflux的cip37地址格式
  ```
