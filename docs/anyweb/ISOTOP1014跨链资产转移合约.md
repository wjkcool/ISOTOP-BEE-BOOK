...
另外写Dapp的前端，使用ISOTOP1014合约时，需要用到` js-conflux-sdk`，`Buffer`和anyweb的`Provider`。
...
```js
    ...
    let  data = Factory.deployContract('ISOTOP1014').data;
...
## 3.  查询子合约地址并获得`IISOTOP`对象
  ...
  ```js
      ...
      const IISOTOP1014Addr = format.address((IISOTOP1014AddrArray[IISOTOP1014AddrArray.length-1]）,1029);  //这里需要将hex40地址格式转换成conflux的cip37地址格式
  ```
...
```js
     data =  IISOTOP.transferFrom(addressfrom,addressto,tokenId,chainId).data;
     recepient = provider.request({
                   method: 'cfx_sendTransaction', params: [{
                                                  from:  address[0], //发送方地址
                                                  to:  IISOTOP1014Addr, //调用的合约地址
