import('./config.js');
const { getContractAddressForNFT1013, call_setApprovalForAll, getOrCreateTokenIdFromAddr } = require('./test_common');
...
describe("测试连接文昌测试链 - NFT1013合约", function(){
    ...
    describe("调用NFT1013合约 - setApprovalForAll", function(){
        ...
        describe("调用 setApprovalForAll 函数 - Success", function() {  
            ...
            it("1.1 调用者为合约Owner，批量转移给合约Owner（自己）", async function() {   
                ...
                try {
                    await call_setApprovalForAll(newContractSub, newContractSub_rw, gas_price, PUBLIC_KEY, PUBLIC_KEY, true);
                } catch (error) {
            ...
            it("1.3 调用者为合约Owner，批量转移合约地址", async function() {   
                ...
                try {
                    await call_setApprovalForAll(newContractSub, newContractSub_rw, gas_price, PUBLIC_KEY, RUN_CONFIG.contractSubAddr, true);
                } catch (error) {
                    console.log("错误信息：", error.message)
            ...
            it("1.4 调用者为非合约Owner，批量转移合约Owner", async function() {   
                ...
                try {
                    await call_setApprovalForAll(newContractSub, newContractSub_rw_not_signer, gas_price, PUBLIC_KEY_IS_NOT_SIGNER, PUBLIC_KEY, true);
                } catch (error) {
                    console.log("错误信息：", error.message)
        ...
        describe("调用 setApprovalForAll 函数 - Error", function() { 
            ...
            it("2.1 调用者为合约Owner，批量转移零地址", async function() {   
                ...
                try {
                    await call_setApprovalForAll(newContractSub, newContractSub_rw, gas_price, PUBLIC_KEY, PUBLIC_KEY_ZERO, false);
                } catch (error) {
                    console.log("错误信息：", error.message)
            ...
            it("2.2 调用者为合约Owner，批量转移无效地址", async function() {   
                ...
                try {
                    await call_setApprovalForAll(newContractSub, newContractSub_rw, gas_price, PUBLIC_KEY, PUBLIC_KEY_INVALID, false);
                } catch (error) {
                    console.log("错误信息：", error.message)
            ...
            it("2.3 调用者为合约Owner，批量转移有效地址， approved非true非false", async function() {   
                //创建1个token   
                tokenId = await getOrCreateTokenIdFromAddr(newContractSub, newContractSub_rw, gas_price, PUBLIC_KEY);        
                
                try {
                    await call_setApprovalForAll(newContractSub, newContractSub_rw, gas_price, PUBLIC_KEY, PUBLIC_KEY, 123);
                } catch (error) {
                    console.log("错误信息：", error.message)
