...
如果你要写Dapp的前端，在使用IISOTOP1011合约时，需要用到`ethers.js`。
...
## 3.  查询子合约地址并获得`IISOTOP`对象
  ...
  ```
之后根据`IISOTOP1011Addr`获得对象`IISOTOP`，就可以调用它的各种方法了
```js
...
查询账户下ID从A-B中包含的NFT使用` IISOTOP.tokensOfOwnerIn(address,start,stop)`方法
  ...
  ```
## 13.销毁NFT
销毁NFT使用function burn(tokenId)方法
  ```js
    await  IISOTOP.burn(tokenId);
  ```
## 14.查看NFT是否存在
查看NFT是否存在使用function exists(tokenId)方法
  ```js
    exist = await  IISOTOP.exists(tokenId);
  ```
## 15.查看NFT销毁总量
查看NFT销毁总量使用function burned()方法
  ```js
    count = await  IISOTOP.burned();
  ```
